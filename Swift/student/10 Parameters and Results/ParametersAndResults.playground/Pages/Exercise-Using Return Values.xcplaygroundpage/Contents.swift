...
func impossibleThingsPhrase( numberOfImpossibleThings: Int , meal: String) -> String {
    ...
    let meal = "teatime"
    let newSentance = "Why, I've believed as many as \(numberOfImpossibleThings) before \(meal)"
    return newSentance
}
...
//:Call `impossibleThingsPhrase`, passing in the result of `impossibleBeliefsCount` as one of the arguments.
 let sentance = impossibleBeliefsCount(pigsFlying: 1, frogsBecomingPrinces: 3, multipleLightningStrikes: 4)

sentance

impossibleThingsPhrase(numberOfImpossibleThings: 1, meal: "lunch")

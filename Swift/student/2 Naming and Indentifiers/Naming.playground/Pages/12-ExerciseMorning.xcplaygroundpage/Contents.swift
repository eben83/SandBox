...
let totalTime = (brushTeeth + drinkCoffee + makeBed + chooseClothes + feedDogs + Shower + run) / 60
...
let thingILike = drinkCoffee + feedDogs + Shower + run
let thingIMustDo = brushTeeth + makeBed + chooseClothes

let newTotalTime = (thingILike + thingIMustDo) / 60
...
*/
let newthingIMustDo = thingIMustDo + 12
let newthingILike = thingILike + 30
let actualTotalTime = (newthingILike + newthingIMustDo) / 60
